...
최종적으로 가장 안전하고 도로 구조를 잘 따르는 경로가 /lattice_path로 선택되어 차량이 이를 추종하게 된다.
...
### 3) 개선 사항
이러한 문제를 해결하기 위해 기존 코드에 다음과 같은 기능을 추가 및 보완하였다:

✅ 도로 우측 경계(/right_path) 정보를 고려하여 경로 이탈 여부 평가
- 어떻게 평가했는가? -> 경로 방향성과 도로 방향이 일치하는지를 벡터 외적(cross product)으로 판단하여 잘못된 방향 경로 필터
```python
노란선 (나가면 안되는 선) 방향: →→→
...
✅ 경로가 도로 경계선에 가까울 경우 가중치를 증가시켜 선택 가능성 낮춤

✅ 객체 중심 기반 회피에서 → 바운딩 박스 또는 클러스터 분산을 반영한 충돌 가능성 평가로 개선

✅ /cluster_distances 토픽을 활용해 클러스터와의 거리 기반 충돌 여부 판단 강화

✅ 

이러한 개선을 통해 차량이 도로 바깥으로 벗어나는 문제를 해결하였고, 실제 MORAI 시뮬레이터에서 도로 중심을 따라 더욱 안정적으로 장애물을 회피하며 주행하는 결과를 얻을 수 있었다.
