        ...
        while not rospy.is_shutdown():
            ...
            if self.is_path and self.is_odom and self.is_status:
                if not self.stop:
                    ...
                    if not self.dynamic:
                        # Change gear to reverse (R)
                        self.send_gear_cmd(Gear.R.value)  # Change to reverse gear (R)
                        rospy.loginfo("Gear changed to R (Reverse)")

                        # Reverse logic (target reverse velocity)
                        self.target_velocity = 10  # Set a reverse speed

                        if not self.stop:
                            # Change gear to drive (D) when stop condition is cleared
                            self.event_cmd.option = 1  # Assuming 1 is the code for drive gear
                            self.send_gear_cmd(Gear.D.value)  # Change to drive gear (D)
                            rospy.loginfo("Gear changed to D (Drive)")
